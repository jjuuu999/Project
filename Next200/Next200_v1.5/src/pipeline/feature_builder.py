...
from src.eligibility import build_eligibility_mask
...
def build_weekly_features(as_of: pd.Timestamp | None = None) -> dict[str, object]:
    ...
    filter_flag = load_table(sql_path, "filter_flag")
    ...
    if not filter_flag.empty:
        ...
        period_flag = filter_flag.loc[
            filter_flag["flag_date"] == target_period,
            ["ticker", "is_managed", "is_warning"],
        ].copy()
        merged = merged.merge(period_flag, on="ticker", how="left")
    ...
    merged["float_rate_source"] = merged.get("float_rate_source", pd.Series(pd.NA, index=merged.index))
    ...
    merged.loc[low_float_mask & merged["sql_float_rate"].notna(), "float_rate_source"] = "historical_fallback"
    merged["float_rate_source"] = merged["float_rate_source"].fillna("naver_actual")
    ...
    merged["rank_change"] = pd.to_numeric(merged["period_rank"], errors="coerce") - merged["prev_rank"]
    ...
    merged["major_holder_source"] = merged.get("major_holder_source", pd.Series(pd.NA, index=merged.index))
    merged["treasury_source"] = merged.get("treasury_source", pd.Series(pd.NA, index=merged.index))
    ...
    merged["treasury_source"] = merged["treasury_source"].fillna("naver_actual")
    ...
    period_row = period_df.loc[period_df["period"].astype(str) == target_period]
    period_end = pd.to_datetime(period_row.iloc[0]["period_end"], errors="coerce") if not period_row.empty else pd.NaT
    merged = merged.loc[~build_eligibility_mask(merged, period_end_date=period_end)].copy()
    ...
    return {
        ...
        "float_rate_source_counts": merged["float_rate_source"].value_counts(dropna=False).to_dict(),
        "major_holder_source_counts": merged["major_holder_source"].value_counts(dropna=False).to_dict(),
        "treasury_source_counts": merged["treasury_source"].value_counts(dropna=False).to_dict(),
    }
